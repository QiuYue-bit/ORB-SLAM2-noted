        ...
        for (int ix = nMinCellX; ix <= nMaxCellX; ix++)
        {
            for (int iy = nMinCellY; iy <= nMaxCellY; iy++)
            {
                ...
                for (size_t j = 0, jend = vCell.size(); j < jend; j++)
                {
                    ...
                    // ! 可能是在圆形之外的，所以只能判断为在这个正方形范围内。
                    // 但是这个也没关系，因为调用这个函数是寻找匹配点，范围大一点也没事~
 ...
 */
     ...
     */
        ...
        // vRowIndices[0] = [1，2，5，8, 11]   第1行有5个特征点,他们的特征点索引值分别为1，2，5，8，11
        // vRowIndices[1] = [2，5，6，9, 13, 17, 20]  第2行有7个特征点.etc
        // 可以看到第一行和第二行都有可能出现特征点2和5
        // nRows 就代表图像有多少行
        ...
        const float maxD = mbf / minZ; // 最大视差对应的距离是相机的基线vslambook 2th P103
        ...
        for (int iL = 0; iL < N; iL++)
        {
            ...
            // 根据左图中索引为IL的特征点所在行，取出右图中可能在vL行上的特征点索引列表
            // 获取左图特征点il所在行，以及在右图对应行中可能的匹配点
            const vector<size_t> &vCandidates = vRowIndices[vL];
            if (vCandidates.empty())
                continue;

            // 根据视差计算理论上的最佳搜索范围 这儿是根据视差范围计算uR
            // uL - uR = d
            ...
            if (bestDist < thOrbDist)
            {
                ...
                // FIXME 其实这里有一个bug，在右图中提取的图像和左图的行是相同的，然而左图的特征点不一定与右图
                // 的特征点在相同的行上，但是这个对于计算视差来说影响不大，所以可以忽略。
                // 这儿主要的不确定度还是在列上，而不是行上。
