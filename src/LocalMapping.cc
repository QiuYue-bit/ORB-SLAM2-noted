...
namespace ORB_SLAM2
{
        ...
        for (size_t i = 0; i < vpMapPointMatches.size(); i++)
        {
            ...
            if (pMP)
            {
                if (!pMP->isBad())
                {
                    ...
                    // 如果这个地图点是当前关键帧新创建的，可能来自双目或RGBD跟踪过程中新生成的地图点，或者是CreateNewMapPoints 中通过三角化产生
                    else
                    {
                        // 那么认为这些新创建的地图点不一定准确，需要进行一轮筛选
    ...
    void LocalMapping::CreateNewMapPoints()
    {
        ...
        {
            // 至少和共视程度最高的帧一起三角化一些地图点
            // 如果队列中有新的关键帧来了，那就先处理新的关键帧去
