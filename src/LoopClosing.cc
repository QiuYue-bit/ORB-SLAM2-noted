...
namespace ORB_SLAM2
{
    ...
    bool LoopClosing::DetectLoop()
    {
        ...
        // Step 3 计算当前帧与共视关键帧(>15个共视地图点)之间的最大相似度，并计算最低得分minScore
        ...
        // 在当前关键帧组和之间的连续关键帧组之间寻找匹配
        mvpEnoughConsistentCandidates.clear();
        vector<ConsistentGroup> vCurrentConsistentGroups;

        // 存储之前的连续关键帧组在当前关键帧组是否连续
        ...
        for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++)
        {
            ...
            // Step 5.1 构建关键帧组，包括候选关键帧及其共视关键帧
            set<KeyFrame *> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames();
            spCandidateGroup.insert(pCandidateKF);

            // Step 5.2 遍历之前的连续关键帧组，寻找连续关系
            ...
            for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++)
            {
                ...
                // step5.3. 若当前连续关键帧组中某关键帧也在前一帧的候选关键帧组中,则找到了连续关系
                ...
                // Step 5.4 找到了连续关系，就更新当前关键帧组的连续次数
                if (bConsistent)
                {
            ...
            // 5.5. 若当前关键帧组在前一关键帧的闭环候选关键帧组中找不到连续关系,则将两虚次数置零
    ...
    bool LoopClosing::ComputeSim3()
    {
            ...
            for (int i = 0; i < nInitialCandidates; i++)
            {
                ...
                if (!Scm.empty())
                {
                    ...
                    if (nInliers >= 20)
                    {
                        ...
                        // gSmw：从世界坐标系 w 到该候选帧pKF m 的Sim3变换，都在一个坐标系下，所以尺度 Scale=1
        ...
        // Step 3：将闭环关键帧及其共视关键帧的所有地图点 投影到 当前关键帧
        ...
        // Step 5: 根据投影成功的地图点数判断Sim3计算的是否准确
        // 超过40个说明成功闭环，否则失败
        int nTotalMatches = 0;
    ...
    void LoopClosing::CorrectLoop()
    {
        ...
        // Step 0：根据共视关系更新当前关键帧与其它关键帧之间的连接关系
