/*
 * @Author: Divenire
 * @Date: 2021-08-19 21:58:05
 * @LastEditors: Divenire
 * @LastEditTime: 2021-11-10 20:51:24
    ...
    int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint *> &vpMapPoints, const float th)
    {
        ...
        for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
        {
            ...
            for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
            {
                ...
                // 因为要寻找的是未匹配上的局部地图点在当前帧的匹配关系，所以有匹配过的点肯定就直接跳过了。
        ...
        int windowSize)
    {
        ...
        for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
        {
            ...
            if (bestDist <= TH_LOW)
            {
                ...
                // 防止特征点过于相似了，决定了特征点的辨识度
                if (bestDist < (float)bestDist2 * mfNNratio)
                {
                    // 如果找到的候选特征点对应F1中特征点已经匹配过了，说明发生了重复匹配，将原来的匹配也删掉
                    // 也不知道哪个匹配是对的，为了保持1对1匹配，把两个撇皮关系都删掉
                    // 这也侧面说明了这个特征点的辨识度是不高的，同时和附近两个点都匹配上了
                    // TODO 调试看看这个实际上发生的频率
 ...
 */
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint *> &sAlreadyFound, const float th, const int ORBdist)
    {
        ...
        const cv::Mat Rcw = CurrentFrame.mTcw.rowRange(0, 3).colRange(0, 3);
        const cv::Mat tcw = CurrentFrame.mTcw.rowRange(0, 3).col(3);
        // 平移t12是坐标系1的原点指向坐标系2原点的向量，在坐标系1下的取值
        // 这儿需要求取当前帧相机光心(原点)在世界坐标系下的取值
