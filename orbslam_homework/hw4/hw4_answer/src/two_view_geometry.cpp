                    ...
                    cv::Mat &F21, float &score, float sigma, int maxIterations)
{
    ...
    std::uniform_int_distribution<int> d(0, vMatches12.size()-1);
    for(int it=0; it<maxIterations; it++)
    {
        ...
        for(int j=0; j<8; j++)
        {
            ...
            vPn1i[j] = vPn1[vMatches12[idx].queryIdx];
            vPn2i[j] = vPn2[vMatches12[idx].trainIdx];
        }
        ...
        currentScore = CheckFundamental(vPts1, vPts2, F21i, vMatches12, vbCurrentInliers, sigma);
                  ...
                  cv::Mat &F21, cv::Mat &K, cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated, float sigma)
{
    ...
    {
        printf("maxGood:%d, nMinGood:%d, nsimilar:%d\n", maxGood, nMinGood, nsimilar);
        return false;
    }
